# Py By @ImranKhan_0_0 🇧🇩 & Enhanced for Telegram Bot
"""
Decrypt Dark Tunnel .dark config exports via Telegram Bot.
Handles: darktunnel://<base64url-json> -> encryptedLockedConfig -> AES-CFB -> msgpack.
With Premium Box Design & Force Join Feature for @minarulsensi
"""
...
from pyrogram.types import Message, Document, InlineKeyboardMarkup, InlineKeyboardButton, CallbackQuery
from pyrogram.errors import UserNotParticipant

# ==================== BOT CONFIGURATION ====================
API_ID = 34808897       # আপনার API ID
API_HASH = "2ab743fe8f005ebea00e9d8c269a1ad3"  # আপনার API Hash
BOT_TOKEN = "8949050319:AAHZiBoZrTw3QvkXDjq6QSjhj52JTtbSPRk" # আপনার Bot Token

CHANNEL_USERNAME = "minarulsensi" # আপনার চ্যানেলের ইউজারনেম (@ বা লিংক ছাড়া)
...
def msgpack_unpack(data: bytes) -> Any:
    ...
    if reader.pos != len(data):
        raise ValueError(f"msgpack has {len(data) - reader.pos} trailing bytes")
    return value
    ...
    if inner_config:
        ...
        for k, v in camel_inner.items():
            if isinstance(v, dict):
                ...
                else: unlocked[tunnel_config_key][k] = v
        ...
        if tunnel_type == "SSH_DNSTT":
            ...
            if inj:
                ...
                if port and port != 53 and port != "53":
        ...
        for k in list(unlocked[tunnel_config_key].keys()):
            ...
            if isinstance(val, dict):
                keys_to_delete = [sub_k for sub_k, sub_v in val.items() if sub_v == ""]
                for sub_k in keys_to_delete:
...
async def check_user_joined(client: Client, user_id: int) -> bool:
    """ব্যবহারকারী চ্যানেলে জয়েন আছে কিনা তা পরীক্ষা করে"""
    try:
        ...
        return True # কোনো টেকনিক্যাল এরর হলে ফাইল প্রসেস করতে দেবে
    return False

def get_force_join_markup() -> InlineKeyboardMarkup:
    """সুন্দর জয়েনিং বাটন লেআউট তৈরি করে"""
...
bot = Client("dark_decryptor_bot", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN)

@bot.on_message(filters.command("start"))
async def start_handler(client: Client, message: Message):
    # স্টার্ট করার সময় Force Join চেক
    if not await check_user_joined(client, message.from_user.id):
        fjoin_text = (
            ...
            "│ অবশ্যই আমাদের চ্যানেলে জয়েন │\n"
            "│ করতে হবে।                    │\n"
            "├──────────────────────────────┤\n"
            "│ নিচের বাটনে ক্লিক করে জয়েন   │\n"
            "│ করুন এবং Check Join 🟢     │\n"
            "│ বাটনে চাপুন।                 │\n"
            "╰──────────────────────────────╯"
        )
        await message.reply_text(fjoin_text, reply_markup=get_force_join_markup())
        return
...
async def core_decrypt_processor(message: Message, content: str, filename: str):
    status_msg = await message.reply_text("⚡ **Processing & Decrypting...**")
    try:
        result = process_dark_config(content)
        
        clean_match = re.split(r'\.dark', filename, flags=re.IGNORECASE)
        base_filename = clean_match[0] if clean_match else "unlocked_config"
        ...
        if os.path.exists(dark_path): os.remove(dark_path)
...
async def file_handler(client: Client, message: Message):
    ...
    if ".dark" in filename:
        # ফাইল প্রসেস করার আগে Force Join চেক
        if not await check_user_joined(client, message.from_user.id):
            fjoin_text = (
                "╭──────────────────────────────╮\n"
                "│    ❌ JOIN CHANNEL FIRST ❌  │\n"
                ...
                "├──────────────────────────────┤\n"
                "│ দয়া করে নিচের চ্যানেল বাটনে  │\n"
                "│ ক্লিক করে জয়েন সম্পন্ন করুন। │\n"
                "╰──────────────────────────────╯"
            )
            await message.reply_text(fjoin_text, reply_markup=get_force_join_markup())
            return
        ...
        await core_decrypt_processor(message, content, message.document.file_name)

# 'Check Join' বাটন ক্লিক হ্যান্ডলার
...
async def callback_check_join(client: Client, callback_query: CallbackQuery):
    ...
    if await check_user_joined(client, user_id):
        ...
        await callback_query.answer("❌ আপনি এখনো জয়েন করেননি! দয়া করে জয়েন করুন।", show_alert=True)
